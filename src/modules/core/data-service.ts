...
export class DataService {
  ...
  loadTileLibrary() {
    ...
    // Store tiles in tileLibrary Map
    Object.entries(this.tiles).forEach(([categoryId, tiles]) => {
      this.tileLibrary.set(categoryId, tiles);
    });
  ...
  getCategoryTiles(categoryId: string): Tile[] {
    return this.tiles[categoryId] || [];
  }

  searchTiles(query: string): (Tile & { categoryId: string; categoryName: string })[] {
    const results: (Tile & { categoryId: string; categoryName: string })[] = [];
  ...
  // Game mode methods
  setGameMode(active: boolean, gameTiles?: any[]): void {
  ...
  addCustomTile(tile: Tile): void {
    // Add custom tile functionality
    const category = tile.categoryId || 'custom';
    if (!this.tiles[category]) {
      this.tiles[category] = [];
    }
    this.tiles[category].push(tile);
  }
}
